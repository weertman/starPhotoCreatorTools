...
if __name__ == '__main__':
    ...
    # Adjust perplexity based on data size (should be smaller than sample size)
    perplexity = min(30, max(5, total_images // 10))

    # Initialize and run t-SNE
    # For small datasets, use exact method. For larger ones, use Barnes-Hut approximation
    method = 'exact' if total_images < 1000 else 'barnes_hut'
    ...
    embedded_coords = tsne.fit_transform(pca_reduced_data)
    ...
    plt.savefig(output_path / 'tsne_embedding.png', dpi=300, bbox_inches='tight')
    # Also save a higher resolution version
    plt.savefig(output_path / 'tsne_embedding_hires.png', dpi=600, bbox_inches='tight')
    plt.close()

    print(f"Visualization saved to {output_path / 'tsne_embedding.png'}")
    print(f"High-resolution version saved to {output_path / 'tsne_embedding_hires.png'}")