...
if __name__ == '__main__':
    ...
    # 2. Embed with PHATE
    print("Running PHATE embedding...")
    # Adjust PHATE parameters based on data size
    knn = min(5, max(2, total_images // 300))
    phate_operator = phate.PHATE(n_components=2, knn=knn, random_state=42)
    embedded_coords = phate_operator.fit_transform(pca_reduced_data)
    ...
    # This gives more control over image sizing
    ...
    plt.savefig(output_path / 'star_embedding.png', dpi=300, bbox_inches='tight')
    # Also save a higher resolution version
    plt.savefig(output_path / 'star_embedding_hires.png', dpi=600, bbox_inches='tight')
    plt.close()

    print(f"Visualization saved to {output_path / 'star_embedding.png'}")
    print(f"High-resolution version saved to {output_path / 'star_embedding_hires.png'}")